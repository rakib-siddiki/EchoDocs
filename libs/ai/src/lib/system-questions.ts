...
export const SYSTEM_QA_PAIRS: SystemQAPair[] = [
  {
    ...
    answer: 'EchoDocs is an advanced document management and Retrieval-Augmented Generation (RAG) platform. It allows users to upload documents (such as PDF and Markdown files), automatically chunks and embeds them using high-dimensional vector representations, and indexes them into a pgvector-enabled database. Users can then query their uploaded documents using a conversational AI interface. The system retrieves relevant document context and feeds it into the Gemini 3.1 Flash Lite model to synthesize precise answers backed by citations to the source materials.',
